...
exports.main = async (event, context) => {
  ...
  for (let part = 1; part <= 6; ++part) {
    ...
    for (let i = 0; i < nums[part]; i++) {
      ...
      if (isEncrypt === true) {
        // do-something
        let salt = "@HZN!U_!2019!@";
        let random_salt = "";
        for (let i = 0; i < 10; ++i) {
          let tmp = String.fromCharCode(Math.floor(Math.random() * 26) + 65);
          random_salt += tmp;
        }
        let encrpt_answer = MD5(salt + arr[ran]["answer"] + random_salt);
        arr[ran]["salt"] = random_salt;
        arr[ran]["answer"] = encrpt_answer;
      }
